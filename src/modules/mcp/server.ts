...
export function buildMcpServer(principal: VerifiedToken): McpServer {
  ...
  if (hasScope(principal, "mcp:write")) {
    ...
    registerTenantTool(
      ...
      {
        description:
          "Update an integration instance (name, enabled, config, credentials, inbound auth). credential_ref/inbound_secret_ref are vault entry NAMES (null clears). Previews a diff and applies NOTHING unless dry_run is false. The `config` shape depends on catalogType. GOOGLE_CALENDAR: {calendarIds(allowlist of calendar ids the agent may operate on; empty ⇒ the tools refuse until one is picked), blockingCalendarIds(calendars only RESPECTED by availability, never operated on, e.g. holidays/closures; EVERY event on them blocks slots; max 10, availability refuses beyond that), calendarLabels(map of id→friendly name), timeZone(IANA, e.g. America/Sao_Paulo), businessHoursId(the BusinessHours profile bounding bookable slots; empty ⇒ no time-of-day limit), slotDurationMinutes, slotGranularityMinutes, appointmentReminders:{enabled,offsetsHours(array of hours-before-start, e.g. [24,1]),askConfirmationOnLast}}. GOOGLE_DRIVE: {folderId, folderName}.",
        inputSchema: {
