...
**Z-PRO channel** (`src/modules/zpro/*`, see `docs/zpro.md`) is a narrower, independent seam set: `stt` (`transcribeZproAudio`, called from `zpro.controller.ts` before dispatch — its own minimal `FlowContext`, since `runtime.ts`'s doesn't exist yet at that point) and `generate` (`runZproAgentTurn`'s graph invoke), correlated by a single `turnId` generated once per webhook delivery in the controller. `vision`/`tts`/`split`/`handoff`/tool lines are **not** wired for Z-PRO yet (no vision/TTS/split pipeline on that channel; INTEGRATION tool calls run but aren't spanned — see `docs/zpro.md`'s "Known, accepted gaps").
